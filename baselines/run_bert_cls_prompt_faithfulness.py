...
def main() -> None:
    ...
    for _, row in tqdm(results_df.iterrows(), total=len(results_df), desc="Faithfulness"):
        ...
        diagnosis_mask_position = None
        if "diagnosis_mask_position" in row and not pd.isna(row["diagnosis_mask_position"]):
            diagnosis_mask_position = int(row["diagnosis_mask_position"])
        ...
        if diagnosis_mask_position is not None and diagnosis_mask_position >= 0:
            ranked_positions = [p for p in ranked_positions if p != diagnosis_mask_position]
        ...
        if diagnosis_mask_position is not None and diagnosis_mask_position >= 0:
            all_reportable_positions.discard(diagnosis_mask_position)
